...
from core_stereo.utils import frame_utils
from core_stereo.utils.augmentor import FlowAugmentor, SparseFlowAugmentor
...
class SceneFlowDatasets(StereoDataset):
    def __init__(self, aug_params=None, root='/mnt/MIG_store/Datasets', dstype='frames_finalpass', things_test=False):
    ...
    def _add_things(self, split='TRAIN'):
        ...
        root = osp.join(self.root, 'flying/flying-things-3d')
    ...
    def _add_driving(self):
        ...
        left_images = sorted( glob(osp.join(root, self.dstype, '*/*/*/left/*.png')) )
        right_images = [ image_file.replace('left', 'right') for image_file in left_images ]
        disparity_images = [ im.replace('frames_finalpass', 'disparity').replace('.png', '.pfm') for im in left_images ]
...
class KITTI(StereoDataset):
    def __init__(self, aug_params=None, root='./data', image_set='training'):
        ...
        disp_list = sorted(glob(os.path.join(root, 'training', 'flow_occ/*_10.png'))) if image_set == 'training' else [osp.join(root, 'training/disp_occ_0/000085_10.png')]*len(image1_list)
...
def fetch_dataloader(args):
    ...
    for dataset_name in args.train_datasets:
        if dataset_name.startswith("middlebury_"):
            ...
            new_dataset = KITTI(aug_params)*200
