...
from raft_stereo import RAFTStereo_Nvim, autocast, RAFTStereo_VSSD
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, default='VSSD', help="which model to use")
    args = parser.parse_args()

    # if args.model=='VSSD':
    model = torch.nn.DataParallel(RAFTStereo_VSSD(args), device_ids=[0])
    # elif args.model=='NVIM':
    #     model = torch.nn.DataParallel(RAFTStereo_Nvim(args), device_ids=[0])

    logging.basicConfig(level=logging.INFO,
                        format='%(asctime)s %(levelname)-8s [%(filename)s:%(lineno)d] %(message)s')

    checkpoint = torch.load('checkpoints_vssd/checkpoints_new_old/26590_epoch_raft-stereo.pth.gz')
