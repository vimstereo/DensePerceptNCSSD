...
import core_stereo.stereo_datasets as datasets
    ...
    while should_keep_training:
        ...
        for i_batch, (_, *data_blob) in enumerate(tqdm(train_loader)):
            ...
            if total_steps % validation_frequency == validation_frequency - 1:
                save_path = Path('checkpoints_mamba2/%d_%s.pth' % (total_steps + 1, args.name))
        ...
        if len(train_loader) >= 5000:
            save_path = Path('checkpoints_disparity/%d_epoch_%s.pth.gz' % (total_steps + 1, args.name))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--transformer_blocks', type=bool , default=False)
    # Training parameters
